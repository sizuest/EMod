...
public class SimulationOutput {
	...
	public SimulationOutput(String filename, ArrayList<MachineComponent> list, List<ASimulationControl> slist) {
		...
		try {
			...
			/* ****** Make file header: ******* */
			/*
			 * 1st Line: Time\tMcName1 Input 1\tMcName1 Input 2\tMcName1 Output
			 * 1\t...
			 */
			outfile.write("Time");
			...
			for (MachineComponent mc : mclist) {
				for (int i = 0; i < mc.getComponent().getInputs().size(); i++) {
					...
					if (mc.getComponent() instanceof Floodable
						...
						outfile.write(separator + mc.getName() + "-Input-"
								+ (i + 1) + "-T");
						outfile.write(separator + mc.getName() + "-Input-"
								+ (i + 1) + "-V");
						outfile.write(separator + mc.getName() + "-Input-"
				...
				for (int i = 0; i < mc.getComponent().getOutputs().size(); i++) {
					...
					if (mc.getComponent() instanceof Floodable
						...
						outfile.write(separator + mc.getName() + "-Output-"
								+ (i + 1) + "-T");
						outfile.write(separator + mc.getName() + "-Output-"
								+ (i + 1) + "-V");
						outfile.write(separator + mc.getName() + "-Output-"
				...
				if (null != mc.getComponent().getDynamicStateList())
					for (int i = 0; i < mc.getComponent().getDynamicStateList()
							.size(); i++) {
						outfile.write(separator + mc.getName() + "-State-"
								+ (i + 1));
					}
			}
			...
			/*
			 * 2nd line:
			 * Time\tMcName1.InputName1\tMcName1.InputName2\tMcName1.OutputName1
			 * \t...
			 */
			outfile.write("   ");
			...
			for (MachineComponent mc : mclist) {
				for (IOContainer input : mc.getComponent().getInputs()) {
					...
					if (mc.getComponent() instanceof Floodable
						...
						outfile.write(separator + mc.getName() + "."
								+ input.getName() + "-Temperature");
						outfile.write(separator + mc.getName() + "."
								+ input.getName() + "-FlowRate");
						outfile.write(separator + mc.getName() + "."
								+ input.getName() + "-Pressure");
					} else
						outfile.write(separator + mc.getName() + "."
								+ input.getName());
				}
				for (IOContainer output : mc.getComponent().getOutputs()) {
					...
					if (mc.getComponent() instanceof Floodable
						...
						outfile.write(separator + mc.getName() + "."
								+ output.getName() + "-Temperature");
						outfile.write(separator + mc.getName() + "."
								+ output.getName() + "-FlowRate");
						outfile.write(separator + mc.getName() + "."
								+ output.getName() + "-Pressure");
					} else
						outfile.write(separator + mc.getName() + "."
								+ output.getName());
				}
				if (null != mc.getComponent().getDynamicStateList())
					for (DynamicState state : mc.getComponent()
							.getDynamicStateList()) {
						outfile.write(separator + mc.getName() + ".State."
								+ state.getName());
					}
			...
			for (MachineComponent mc : mclist) {
				for (IOContainer input : mc.getComponent().getInputs()) {
					...
					if (mc.getComponent() instanceof Floodable
							& input instanceof FluidContainer) {
						outfile.write(separator + "["
								+ (new SiUnit(Unit.KELVIN)).toString() + "]");
						outfile.write(separator + "["
								+ (new SiUnit(Unit.METERCUBIC_S)).toString()
								+ "]");
						outfile.write(separator + "["
								+ (new SiUnit(Unit.PA)).toString() + "]");
					} else
						outfile.write(separator + "["
								+ input.getUnit().toString() + "]");
				}
				for (IOContainer output : mc.getComponent().getOutputs()) {
					if (output.hasReference())
						continue;
					if (mc.getComponent() instanceof Floodable
						...
						outfile.write(separator + "["
								+ output.getUnit().toString() + "]");
				}
				if (null != mc.getComponent().getDynamicStateList())
					for (DynamicState state : mc.getComponent()
							.getDynamicStateList()) {
						outfile.write(separator + "["
								+ state.getUnit().toString() + "]");
					}
			}
			for (ASimulationControl sc : simlist) {
				outfile.write(separator + "       ");
			...
			for (MachineComponent mc : mclist) {
				for (IOContainer input : mc.getComponent().getInputs()) {
					...
					if (mc.getComponent() instanceof Floodable
							& input instanceof FluidContainer) {
						outfile.write(separator
								+ ((FluidContainer) input).getTemperature());
						outfile.write(separator
								+ ((FluidContainer) input)
										.getFluidCircuitProperties()
										.getFlowRate());
						outfile.write(separator
								+ ((FluidContainer) input).getPressure());
					} else
				...
				for (IOContainer output : mc.getComponent().getOutputs()) {
					...
					if (mc.getComponent() instanceof Floodable
							& output instanceof FluidContainer) {
						outfile.write(separator
								+ ((FluidContainer) output).getTemperature());
						outfile.write(separator
								+ ((FluidContainer) output)
										.getFluidCircuitProperties()
										.getFlowRate());
						outfile.write(separator
								+ ((FluidContainer) output).getPressure());
					} else
