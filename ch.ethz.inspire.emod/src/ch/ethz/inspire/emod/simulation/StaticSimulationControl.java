...
/**
 * reads static simulation samples from a file and loops while active. may be
 * only one value.
 * 
 * @author dhampl
 * 
...
public class StaticSimulationControl extends ASimulationControl {
	...
	private static Logger logger = Logger
			.getLogger(StaticSimulationControl.class.getName());
	 ...
	 * 
	 * @param file
	 *            one line per state. e.g.: ON=10 20 30
	 */
	private void readSamplesFromFile() {
		samples = new ArrayList<double[]>();
		logger.log(LogLevel.DEBUG, "reading samples for: "
			...
			try {
				for (int i = 0; i < samples.size(); i++) {
					samples.set(i, SamplePeriodConverter.convertSamples(
							simulationPeriod, simulationPeriod, samples.get(i),
							true));
				}
			} catch (Exception ex) {
				ex.printStackTrace();
