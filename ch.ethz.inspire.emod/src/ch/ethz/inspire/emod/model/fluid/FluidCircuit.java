...
public class FluidCircuit {
	...
	public static void floodCircuit(FluidContainer source, FluidContainer target) {
		...
		source.getFluidCircuitProperties().setPost(
				target.getFluidCircuitProperties());
	}
	...
	public void checkCircuit() {
		...
		while (count <= mcList.size() && !connected) {
			...
			for (IOConnection io : ioList) {
				...
				if (io.getSource().equals(
						source.getComponent().getOutput("FluidOut"))) {
					// search the list of machine components
					for (MachineComponent mc : mcList) {
						// and get the component that is the target of the
						// connection
						if (io.getTarget().equals(
								mc.getComponent().getInput("FluidIn"))) {
							// go one step further, i.e. the old target is the
							// new source
							source = mc;
							// if the target is the tank, the circuit is
							// connected
							if (mc.getComponent() instanceof ch.ethz.inspire.emod.model.Tank) {
								connected = true;
