...
/**
 * Implements a solver for hydraulic circuit problems
 * 
 * @author sizuest
 *
 */
public class FluidCircuitSolver {
	...
	private ArrayList<Integer> preE = new ArrayList<Integer>(),
			postE = new ArrayList<Integer>(), bcinE = new ArrayList<Integer>(),
	 ...
	 * @param connections
	 */
	public FluidCircuitSolver(
			ArrayList<FluidCircuitProperties> fluidPropertyList,
			List<FluidConnection> connections) {
	...
	public void init() {
		...
		/*
		 * SpreB.print(); SpostB.print(); Mpre.print(); Mpost.print();
		 */

		// Spre
		Spre = new SimpleMatrix(numC, numE);
		for (int i = 0; i < numC; i++) {
			int idxE = fluidPropertyList.indexOf(((FluidContainer) (connections
					.get(i).getTarget())).getFluidCircuitProperties());
			Spre.set(i, idxE, 1);
		}
		...
		for (int i = 0; i < numC; i++) {
			int idxE = fluidPropertyList.indexOf(((FluidContainer) (connections
					.get(i).getSource())).getFluidCircuitProperties());
			Spost.set(i, idxE, 1);
		}
		...
		H = new SimpleMatrix(numE + numCPre + numCPost + 2 * numC + numBCIn
				+ numBCOut, 3 * numE + 2 * numC);
		...
		H.insertIntoThis(numE + numCPre + numCPost, 3 * numE + numC,
				SimpleMatrix.identity(numC).negative());
		H.insertIntoThis(numE + numCPre + numCPost + numC, 2 * numE, Spost);
		H.insertIntoThis(numE + numCPre + numCPost + numC, 3 * numE + numC,
				SimpleMatrix.identity(numC).negative());

		// Boundary conditions (Pressure)
		H.insertIntoThis(numE + numCPre + numCPost + 2 * numC, numE, Mbcin);
		H.insertIntoThis(numE + numCPre + numCPost + 2 * numC + numBCIn,
				2 * numE, Mbcout);
		...
		for (int i = 0; i < numE; i++) {
			...
			for (int j = 0; j < idx.size(); j++) {
				idx.set(j, fluidPropertyList
						.indexOf(((FluidContainer) connections.get(idx.get(j))
								.getSource()).getFluidCircuitProperties()));
			}
		...
		for (int i = 0; i < numE; i++) {
			...
			for (int j = 0; j < idx.size(); j++) {
				idx.set(j, fluidPropertyList
						.indexOf(((FluidContainer) connections.get(idx.get(j))
								.getTarget()).getFluidCircuitProperties()));
			}
	...
	public void solve(int maxIterations) throws Exception {
		...
		// Read new boundaries
		for (int i = 0; i < numBCIn; i++)
			prefIn[i] = fluidPropertyList.get(bcinE.get(i))
					.getPressureReferenceIn();

		for (int i = 0; i < numBCOut; i++)
			prefOut[i] = fluidPropertyList.get(bcoutE.get(i))
					.getPressureReferenceOut();
		...
		while (iteration < maxIterations & relChange > 1E-4) {
			...
			// Read new op
			for (int i = 0; i < numE; i++) {
				a0[i] = fluidPropertyList
						.get(i)
						.getCharacterisitc()
						.getA0(fluidPropertyList.get(i).getFlowRate(),
								fluidPropertyList.get(i).getPressureIn(),
								fluidPropertyList.get(i).getPressureOut());
				a1[i] = fluidPropertyList
						.get(i)
						.getCharacterisitc()
						.getA1(fluidPropertyList.get(i).getFlowRate(),
								fluidPropertyList.get(i).getPressureIn(),
								fluidPropertyList.get(i).getPressureOut());
				e[i] = fluidPropertyList
						.get(i)
						.getCharacterisitc()
						.getEp(fluidPropertyList.get(i).getFlowRate(),
								fluidPropertyList.get(i).getPressureIn(),
								fluidPropertyList.get(i).getPressureOut());
			}
			...
			// Write back data
			for (int i = 0; i < numE; i++) {
				fluidPropertyList.get(i).setPressureIn(
						Math.round(sol.get(i + numE)));
				fluidPropertyList.get(i).setPressureOut(
						Math.round(sol.get(i + 2 * numE)));
		...
		if (iteration >= maxIterations) {
			System.out
					.println("Warning: Fluid circuit solution didn't coverged. Max change rate: "
							+ relChange);
		}
	}

	private SimpleMatrix solveLinearSystem(double[] a1, double[] a0,
			double[] e, double[] pBCIn, double[] pBCOut) throws Exception {
		SimpleMatrix y = new SimpleMatrix(numE + 2 * numC + numCPre + numCPost
				+ numBCIn + numBCOut, 1);
	...
	private ArrayList<Integer> getPostIndexes(FluidCircuitProperties p) {
		...
		for (int i = 0; i < connections.size(); i++)
			if (((FluidContainer) (connections.get(i).getSource()))
					.getFluidCircuitProperties().equals(p))
				out.add(i);
	...
	/**
	 * Returns the number of missing BC
	 * @return
	 */
