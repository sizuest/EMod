...
public class Tank extends APhysicalComponent implements Floodable {
	...
	private void init() {
		...
		fluidPropertiesOut = new FluidCircuitProperties(new FECIdeal(),
		...
		temperatureTank = new IOContainer("TemperatureTank", new SiUnit(
				Unit.KELVIN), temperatureInit, ContainerType.THERMAL);
		...
		try {
			...
			fluid.getMass().setInitialCondition(
					volume / fluid.getMaterial().getDensity(293.15));
		...
		fluidIn = new FluidContainer("FluidIn", new SiUnit(Unit.NONE),
				ContainerType.FLUIDDYNAMIC, fluidPropertiesIn);
		inputs.add(fluidIn);
		// TODO manick: test for Fluid
		fluidOut = new FluidContainer("FluidOut", new SiUnit(Unit.NONE),
				ContainerType.FLUIDDYNAMIC, fluidPropertiesOut);
	...
	public IOContainer getInput(String name) {
		...
		if (name.equals(fluidIn.getName())) {
			temp = new FluidContainer("FluidIn-" + (inputsDyn.size() + 1),
					fluidIn, fluidPropertiesIn);
			inputs.add(temp);
	...
	public IOContainer getOutput(String name) {
		...
		if (name.equals(fluidOut.getName())) {
			temp = new FluidContainer("FluidIOut-" + (outputsDyn.size() + 1),
					fluidOut, fluidPropertiesOut);
	...
	public void update() {
		...
		/* Convection */
		alphaFluid = Fluid.convectionFreeCuboid(new Material("Air"), fluid
		...
		fluid.integrate(timestep, flowRateIn, flowRateOut,
				pressureAmb.getValue());
