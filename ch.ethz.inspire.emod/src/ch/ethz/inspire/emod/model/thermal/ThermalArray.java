...
public class ThermalArray extends AThermalIntegrator {
	...
	public void setThermalResistance(double thermalResistance) {
		if (!(Double.isInfinite(thermalResistance) | Double
				.isNaN(thermalResistance)))
	...
	public void setHeatSource(double heatSource) {
		if (!(heatSource == 0 | Double.isInfinite(heatSource) | Double
				.isNaN(heatSource)))
	...
	public double getHeatLoss() {
		if (thermalResistance == 0 | Double.isInfinite(thermalResistance)
	...
	public String toString() {
		return tempAmb + " " + tempIn + " " + material.toString() + " "
				+ volume + " " + flowRate + " " + pressure + " " + heatSource
				+ " " + numElements;
	}
	...
	public double getA(int i) {
		...
		return -(cp * massFlowRate * numElements + thermalResistance)
				/ massState.getValue() / cp;
	}

	@Override
	public double getB(int i) {
		// B[k] = (mDotIn[k]*cpIn[k]*Tin[k]*N +
		// Tamb[k]*Rth[k]+heatInput[k])/m[k]/cp[k]
		double cp = material.getHeatCapacity();
		double cpIn = material.getHeatCapacity();
		double tempIn = (0 == i) ? this.tempIn : temperature.get(i - 1)
				.getCurrent();

		return (cpIn * tempIn * massFlowRate + tempAmb * thermalResistance
				/ numElements + heatSource / numElements)
				* numElements / massState.getValue() / cp;
	}
}
