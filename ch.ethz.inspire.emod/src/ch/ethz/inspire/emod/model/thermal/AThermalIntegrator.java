 ...
 * 
 * dm/dt = mDotIn + mDotOut dTi/dt = Ai*Ti+Bi
 * 
 * where - T = [T1 T2 ... TN]^T - u = [u1 u2 ... uM]^T - Ai = Ai(T1, T2, ..,
 * Ti-1, m, u1, u2, ... uM) - Bi = Bi(T1, T2, .., Ti-1, m, u1, u2, ... uM)
 * 
...
public abstract class AThermalIntegrator {
	...
	protected void init() {
		...
		temperatureState = new DynamicState("Temperature", new SiUnit(
				Unit.KELVIN), 293.15);
		temperatureOutState = new DynamicState("TemperatureOut", new SiUnit(
				Unit.KELVIN), 293.15);
		try {
			temperatureState.setInitialConditionFunction(this.getClass()
					.getMethod("setInitialTemperature", double.class), this);
		} catch (Exception e) {
			System.err
					.println("Dynamic State: couldn't assosiate init function!");
			e.printStackTrace();
		}
	...
	/**
	 * Used for the update of Ai. Is called before each iteration step. Must
	 * return the new Ai at the current time step
	 * 
	 * @param i
	 * @return A
	 */
	...
	/**
	 * Used for the update of Bi. Is called before each iteration step. Must
	 * return the new Bi at the current time step
	 * 
	 * @param i
	 * @return B
	 */
	 ...
	 *            [Pa]
	 */
	public final void integrate(double timestep, double flowRateIn,
		...
		massState.setValue(massState.getValue()
				+ timestep
				/ 2
				* (mDotIn.getCurrent() + mDotIn.getLast()
						- mDotOut.getCurrent() - mDotOut.getLast()));
		...
		for (int i = 0; i < numElements; i++) {
			...
			this.temperature
					.get(i)
					.update(((this.temperature.get(i).getCurrent()
							* (1 + timestep / 2 * A.get(i).getLast()) + timestep
							/ 2 * (B.get(i).getCurrent() + B.get(i).getLast())) / (1 - timestep
							/ 2 * A.get(i).getCurrent())));
		}

		/* Bulk temperature as system state */
		temperatureState.setValue(getTemperatureBulk());
		temperatureOutState.setValue(temperature.get(temperature.size() - 1)
				.getCurrent());
	}
	...
	/**
	 * Performs a single integration step of length 'timestep' without mass
	 * flows
	 * 
	 * @param timestep
	 */
	...
	protected double getTemperatureBulk() {
		...
		for (int i = 0; i < numElements; i++) {
			...
			H += massState.getValue() / numElements * cp
					* temperature.get(i).getCurrent();
		}
