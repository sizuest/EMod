 ...
 * 
 * Config parameters: Surface : [m^2] : Surface between level 1 and 2
 * ConvectionConstants : [W/K/m^2] : Heat transfer constants for convection on
 * side 1 and 2 ConductionConstants : [W/K/m] : Heat transfer constants for
 * conduction WallThicknesses : [m] : Thicknesses of the wall layers
 * 
 * 
 * @author simon
 * 
...
public class FreeHeatTransfere extends APhysicalComponent {
	...
	private void init() {
		...
		/*
		 * Calculate thermal ressistance k, with cases: - lambda=alpha=0 k = 0 -
		 * alpha=0 k=lambda/d; - lambda=0 k=alpha; - else k =
		 * (1/alpha+d/lambda)^-1
		 */
		thRessistance = 0;
	...
	public void update() {
		...
		/*
		 * The heat transfere from 1 to 2 is Qdot12 [W] = k [W/m²] * A [m²] *
		 * (T_2 - T_1)
		 */

		pth12.setValue(thRessistance * surf
