...
public class Pump extends APhysicalComponent implements Floodable {
	...
	private double[] powerSamplesV, pressureSamplesV, flowRateSamplesV,
			effPumpSamplesV;
	...
	private void init() {
		...
		pumpCtrl = new IOContainer("State", new SiUnit(Unit.NONE), 0,
		...
		pel = new IOContainer("PTotal", new SiUnit(Unit.WATT), 0.00,
				ContainerType.ELECTRIC);
		pth = new IOContainer("PLoss", new SiUnit(Unit.WATT), 0.00,
				ContainerType.THERMAL);
		pmech = new IOContainer("PUse", new SiUnit(Unit.WATT), 0.00,
				ContainerType.FLUIDDYNAMIC);
		heatFlowAmbient = new IOContainer("HeatFlowAmbient", new SiUnit(
				Unit.WATT), 0, ContainerType.THERMAL);
		heatFlowTank = new IOContainer("HeatFlowTank", new SiUnit(Unit.WATT),
				0, ContainerType.THERMAL);
		outputs.add(pel);
		outputs.add(pth);
		outputs.add(pmech);
		...
		bcMotorHeatSrc = new BoundaryCondition("HeatSrcMotor", new SiUnit("W"),
				0, BoundaryConditionType.NEUMANN);
		bcMotorHTC = new BoundaryCondition("MotorHTC", new SiUnit("W/K"), 0,
				BoundaryConditionType.ROBIN);
		bcPumpHeatSrc = new BoundaryCondition("HeatSrcPump", new SiUnit("W"),
				0, BoundaryConditionType.NEUMANN);
		bcPumpHTC = new BoundaryCondition("PumpHTC", new SiUnit("W/K"), 0,
				BoundaryConditionType.ROBIN);
		...
		try {
			...
			for (int i = flowRateSamples.length - 1; i >= 0; i--) {
				if (0 == flowRateSamples[i] & flowRateSamples.length - i > 1)
					...
					effMotorSamples[i] = pressureSamples[i]
							* flowRateSamples[i] / powerSamples[i]
							/ effPumpSamples[i];
			}
			...
			for (int i = 0; i < effPumpSamples.length; i++)
				lossMax = Math.max(powerSamples[i] * (1 - effMotorSamples[i]),
						lossMax);
		...
		fluidProperties = new FluidCircuitProperties(
				new FECPump(this, pumpCtrl), fluid.getTemperature());
		...
		fluidOut.getFluidCircuitProperties().setTemperature(
		...
		if (pressureSamples.length != flowRateSamples.length) {
			throw new Exception(
					"Pump, type:"
							...
							+ "'flowRateSamples' (" + pressureSamples.length
							+ "!=" + flowRateSamples.length + ")!");
		}
		if (pressureSamples.length != powerSamples.length) {
			throw new Exception(
					"Pump, type:"
							+ type
							+ ": Dimension missmatch: Vector 'pressureSamples' must have same dimension as "
							+ "'powertSamples' (" + pressureSamples.length
							+ "!=" + powerSamples.length + ")!");
		}
		// Check if sorted:
	...
	public void update() {
		...
		if (pumpCtrl.getValue() > 0 & hasMotorCooling)
			...
			htcMotor = Fluid.convectionFreeCylinderVert(new Material("Air"),
					structure.getTemperature().getValue(),
					temperatureAmb.getValue(), lengthMotor, diameterMotor);

		if (isSubmerged)
			htcPump = Fluid.convectionFreeCylinderVert(fluid.getMaterial(),
					fluid.getTemperature().getValue(),
					fluidIn.getTemperature(), lengthPump, diameterPump);
		else
		...
		fluid.integrate(timestep, fluidProperties.getFlowRate(),
				fluidProperties.getFlowRate(), fluidProperties.getPressure());
	...
	private void updatePumpMap(double rho, double nu) {
		...
		if (numImpEyes == 0 | numStages == 0) {
			...
			/* Univ. spec. speed */
			omegaS = omega * Math.sqrt(flowRateBEP / numImpEyes)
					/ Math.pow(pressureBEP / 1000 / numStages, .75);
			...
			for (int i = 0; i < flowRateSamples.length; i++) {
				...
				if (flowRateSamples[i] == 0)
					...
					powerSamplesV[i] = flowRateSamplesV[i]
							* pressureSamplesV[i] / effPumpSamplesV[i]
							/ effMotorSamples[i];
	...
	public double getPressure(double flowRate) {
		...
		// return 0;
		return Algo.linearInterpolation(flowRate, flowRateSamplesV,
				pressureSamplesV);
	}
	...
	public double getPressureDrivative(double flowRate) {
		return Algo.numericalDerivative(flowRate, flowRateSamplesV,
				pressureSamplesV);
	}
