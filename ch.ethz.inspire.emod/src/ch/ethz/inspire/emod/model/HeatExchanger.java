	...
	public void update() {
		...
		if (!isInitialized) {
			fluid1.getMass().setInitialCondition(
					volume1
							* fluid1.getMaterial().getDensity(
									fluid1.getTemperature().getValue()));
			fluid2.getMass().setInitialCondition(
					volume2
							* fluid2.getMaterial().getDensity(
									fluid2.getTemperature().getValue()));
			isInitialized = true;
		}
		...
		if (level.getValue() == 1) {
			...
			if (tempOff == tempOn) {
				...
				heatFlux = Math.max(0, (fluid1In.getTemperature() - tempOn)
						* fluidProperties1.getMassFlowRate()
		...
		fluid1.integrate(timestep, fluidProperties1.getFlowRate(),
				fluidProperties1.getFlowRate(), fluidProperties1.getPressure());
		fluid2.integrate(timestep, fluidProperties2.getFlowRate(),
				fluidProperties2.getFlowRate(), fluidProperties2.getPressure());
