...
public class EModStartupGUI {
	...
	protected static Shell loadMachineGUI(Shell parent) {
		...
		// the possible selection of the comboMachineConfigName has to change
		...
		textLoadSimConfig.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				true, false, 1, 1));
		textLoadSimConfig.setText(LocalizationHandler
				.getItem("app.gui.startup.simulationconfigname"));
		...
		textLoadProcConfig.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				true, false, 1, 1));
		textLoadProcConfig.setText(LocalizationHandler
				.getItem("app.gui.startup.processconfigname"));
		...
		buttonNew.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true, false,
				1, 1));
		buttonNew.setText(LocalizationHandler
				.getItem("app.gui.startup.newmachine"));
		buttonNew.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent event) {
				// open new window to input the desired machine name and config
				// name
				createNewMachineGUI();
		...
		buttonContinue.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, true,
				false, 1, 1));
		...
		buttonContinue.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent event) {
				// get the values for the Machine Name and the Machine Config
				// Name
				...
				if (machConfig.equals(LocalizationHandler
	...
	protected static void createNewMachineGUI() {
		final Shell shellCreateMachine = new Shell(Display.getCurrent(),
				SWT.APPLICATION_MODAL);
		shellCreateMachine.setText("EMod startup");
		shellCreateMachine.setLayout(new GridLayout(2, true));

		// text load machine config
		Text textMachineName = new Text(shellCreateMachine, SWT.READ_ONLY
				| SWT.LEFT);
		textMachineName.setLayoutData(new GridData(GridData.FILL,
				GridData.CENTER, true, false, 1, 1));
		textMachineName.setText(LocalizationHandler
				.getItem("app.gui.startup.newmachinename"));

		// text for the user to enter the desired Machine Name
		final Text textMachineNameValue = new Text(shellCreateMachine, SWT.NONE);
		textMachineNameValue.setLayoutData(new GridData(GridData.FILL,
				GridData.CENTER, true, false, 1, 1));

		// text load machine config
		Text textMachineConfigName = new Text(shellCreateMachine, SWT.READ_ONLY
				| SWT.LEFT);
		textMachineConfigName.setLayoutData(new GridData(GridData.FILL,
				GridData.CENTER, true, false, 1, 1));
		textMachineConfigName.setText(LocalizationHandler
				.getItem("app.gui.startup.newmachineconfigname"));

		// text for the user to enter the desired Machine Name
		final Text textMachineConfigNameValue = new Text(shellCreateMachine,
				SWT.NONE);
		textMachineConfigNameValue.setLayoutData(new GridData(GridData.FILL,
				GridData.CENTER, true, false, 1, 1));

		// Button to continue (exit the window, load the selected configuration)
		Button buttonContinue = new Button(shellCreateMachine, SWT.NONE);
		GridData gridData = new GridData(GridData.END, GridData.CENTER, true,
				false);
		...
		int[] position;
		position = EModGUI.shellPosition();
		shellCreateMachine.setLocation(position[0] - size[0] / 2, position[1]
				- size[1] / 2);
	 ...
	 * 
	 * @param machine
	 *            name of the machine to add
	 * @param config
	 *            name of the machine conifguration to add
	 */
