...
/**
 * GUI to display the material GUI and sleect materials
 * @author sizuest
 *
 */
public class MaterialDBGUI {
	...
	public MaterialDBGUI(Shell parent) {
		...
		buttonEdit
				.setText(LocalizationHandler.getItem("app.gui.matdb.editmat"));
		buttonEdit.setLayoutData(new GridData(SWT.FILL, SWT.END, true, false,
				1, 1));
		// selection listener for the button
		buttonEdit.addSelectionListener(new SelectionListener() {
			// when selected -> get selection -> get file of component -> open
			// window to edit said component
			@Override
		...
		buttonClose.setLayoutData(new GridData(SWT.FILL, SWT.END, true, false,
				1, 1));
