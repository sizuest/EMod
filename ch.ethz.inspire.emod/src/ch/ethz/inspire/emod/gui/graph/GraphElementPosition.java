 ...
 * 
 * Implements the position of a graph element. This includes the x and y coordinate, as
 * well as the coordinate transformation between the SWT and Graph coordinate systems
 * 
 * @author sizuest
 *
 */
...
public class GraphElementPosition {
	 ...
	 * @param p			SWT coordinates
	 * @param parent	Graph parent
	 */
	 ...
	 * set
	 * 
	 * Sets the point (Graph coordinate system).
	 * If not called by the apropriate constructor, this method must not be used with SWT coordinates!
	 * @param p 
	 */
	...
	public Point2D get(PSWTCanvas parent){
		...
		double rx = this.x/(double)bounds.width,
		       ry = this.y/(double)bounds.height;
		
		// Absolute position in the canvas
		Point2D point = new Point2D.Double(viewBounds.x+viewBounds.width*rx, 
				                           viewBounds.y+viewBounds.height*ry);
