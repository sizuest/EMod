...
/**
 * Edit simulation input properties
 * 
 * @author sizuest
 *
 */
public class EditInputGUI extends AConfigGUI {
	 ...
	 * @param parameter
	 */
	public EditInputGUI(Composite parent, int style, String type,
			String parameter) {
	...
	private void init() {
		...
		labelName.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false,
				1, 1));

		textName = new Text(this.getContent(), SWT.BORDER);
		textName.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 1,
				1));
		...
		labelUnit.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false,
				1, 1));

		comboUnit = new Combo(this.getContent(), SWT.NONE);
		comboUnit.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false,
				1, 1));
		...
		if (sc instanceof StaticSimulationControl)
			inputComposite = new EditStaticSimulationControlGUI(
					this.getContent(), SWT.NONE, sc);
		else if (sc instanceof ProcessSimulationControl)
			inputComposite = null;
		else if (sc instanceof GeometricKienzleSimulationControl) {
			inputComposite = new EditGeometricKienzleSimulationControlGUI(
					this.getContent(), SWT.NONE, sc);
			comboUnit.setEnabled(false);
		} else
			inputComposite = new EditGenergicSimulationControlGUI(
					this.getContent(), SWT.NONE, sc);
