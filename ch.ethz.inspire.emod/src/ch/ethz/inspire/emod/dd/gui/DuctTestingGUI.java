...
public class DuctTestingGUI extends AGUITab {
	...
	private ILineSeries lineSeriesPressureTesting, lineSeriesHTCTesting,
			lineSeriesTemperatureFluid, lineSeriesTemperatureWall,
	...
	private double flowRate = 9.3E-5, pressure = 3e5, temperatureF = 293.15;
	...
	public void init() {
		...
		tabConfig.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 1,
				4));

		/* OP Table */
		tableOpPoint = new Table(tabConfig, SWT.BORDER | SWT.MULTI
				| SWT.V_SCROLL);
		tableOpPoint.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true,
				1, 1));
		...
		String[] titlesBC = {
				...
				LocalizationHandler
						.getItem("app.dd.testing.gui.bc.temperature")
						+ " ["
						+ (new SiUnit("K")).toString() + "]",
				LocalizationHandler.getItem("app.dd.testing.gui.bc.heatFlux")
						+ " [" + (new SiUnit("W")).toString() + "]", "        " };
		...
		String[] titlesTest = {
				...
				"ζ ["
						+ SiUnit.divide(new SiUnit("Pa"),
								SiUnit.pow(new SiUnit(Unit.METERCUBIC_S), 2))
								.toString() + "]",
		...
		chartTesting
				...
				.setText(
						LocalizationHandler
								.getItem("app.dd.testing.gui.temperature")
								+ " [" + (new SiUnit("K")).toString() + "]");
		chartTesting.getAxisSet().getYAxis(2).getTick()
				.setForeground(colorTemperature);
		chartTesting.getAxisSet().getYAxis(2).getTitle()
				.setForeground(colorTemperature);
		...
		lineSeriesHTCTesting = (ILineSeries) chartTesting.getSeriesSet()
		...
		lineSeriesPressureTesting = (ILineSeries) chartTesting.getSeriesSet()
				.createSeries(
						SeriesType.LINE,
						LocalizationHandler
								.getItem("app.dd.testing.gui.pressure"));
		...
		lineSeriesTemperatureFluid = (ILineSeries) chartTesting
				.getSeriesSet()
				.createSeries(
						SeriesType.LINE,
						LocalizationHandler
								.getItem("app.dd.testing.gui.temperature.fluid"));
		...
		lineSeriesTemperatureWall = (ILineSeries) chartTesting
				.getSeriesSet()
				.createSeries(
						SeriesType.LINE,
						LocalizationHandler
								.getItem("app.dd.testing.gui.temperature.bulk"));
		...
		chartCC.getAxisSet()
				...
				.setText(
						LocalizationHandler
								.getItem("app.dd.testing.gui.flowrate")
								+ " ["
								+ (new SiUnit("m^3 s^-1")).toString() + "]");
		chartCC.getAxisSet().getXAxis(0).getTick()
				.setForeground(Display.getDefault().getSystemColor(0));
		chartCC.getAxisSet().getXAxis(0).getTitle()
				.setForeground(Display.getDefault().getSystemColor(0));
		chartCC.getAxisSet().getXAxis(0).getTick()
		...
		lineSeriesPressureCC = (ILineSeries) chartCC.getSeriesSet()
				.createSeries(
						SeriesType.LINE,
						LocalizationHandler
								.getItem("app.dd.testing.gui.pressure"));
		...
		tabConfigFlow.setText(LocalizationHandler
				.getItem("app.dd.testing.gui.fluidproperties"));
		tabConfigFlow.setToolTipText("");
		tabConfigFlow.setControl(tableOpPoint);

		TabItem tabConfigBC = new TabItem(tabConfig, SWT.NONE);
		tabConfigBC.setText(LocalizationHandler
				.getItem("app.dd.testing.gui.bc"));
		tabConfigBC.setToolTipText("");
		tabConfigBC.setControl(tableBC);

		TabItem tabAnalysisNum = new TabItem(tabAnalysis, SWT.NONE);
		tabAnalysisNum.setText(LocalizationHandler
				.getItem("app.dd.testing.gui.analysis.numerical"));
		...
		tabAnalysisPlot.setText(LocalizationHandler
				.getItem("app.dd.testing.gui.analysis.graphical"));
		...
		tabCharacteristicsPlot.setText(LocalizationHandler
				.getItem("app.dd.testing.gui.analysis.characteristicdiag"));
		tabCharacteristicsPlot.setToolTipText("");
		tabCharacteristicsPlot.setControl(chartCC);

		/* Add editor and cp */
		try {
			TableUtils.addCellEditor(tableOpPoint, this.getClass()
					.getDeclaredMethod("setOperationalPoint"), this,
					new int[] { 1 });
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			TableUtils.addCellEditor(tableBC, this.getClass()
					.getDeclaredMethod("setBoundaryConditions"), this,
					new int[] { 1, 2 });
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		itemPrint
				.setText(LocalizationHandler.getItem("app.gui.file.saveasimg"));
		...
		itemAddPump
				.setText(LocalizationHandler
						.getItem("app.dd.testing.gui.analysis.characteristicdiag.addpump"));
		...
		itemPrintTesting.setText(LocalizationHandler
				.getItem("app.gui.file.saveasimg"));
	...
	private void readOPTable() {
		for (int i = 0; i < tableOpPoint.getItemCount(); i++)
			try {
				switch (i) {
				case 0:
					this.flowRate = Double.parseDouble(tableOpPoint.getItem(i)
							.getText(1));
					break;
				case 1:
					this.pressure = Double.parseDouble(tableOpPoint.getItem(i)
							.getText(1));
					break;
				case 2:
					this.temperatureF = Double.parseDouble(tableOpPoint
							.getItem(i).getText(1));
					break;
				case 3:
					duct.setMaterial(new Material(tableOpPoint.getItem(i)
							.getText(1)));
				}
			} catch (Exception e) {
				// Not used
	...
	private void readBCTable() {
		...
		for (int i = 0; i < tableBC.getItemCount(); i++) {
			...
			curElement = duct.getElement(tableBC.getItem(i).getText(0)
					.replaceAll("\\s*[0-9]:\\s*", ""));

			// First try: wall temperature
			try {
				curElement.setWallTemperature(Double.parseDouble(tableBC
						.getItem(i).getText(1)));
			} catch (Exception e1) {
				...
				try {
					curElement.setHeatSource(Double.parseDouble(tableBC
							.getItem(i).getText(2)));
				} catch (Exception e2) {
	...
	private void updateOPTable() {
		...
		itemFlowRate.setText(0,
				LocalizationHandler.getItem("app.dd.testing.gui.flowrate"));
		itemFlowRate.setText(1, this.flowRate + "");
		itemFlowRate.setText(2, (new SiUnit("m^3/s")).toString());

		itemPressure.setText(0,
				LocalizationHandler.getItem("app.dd.testing.gui.pressurein"));
		itemPressure.setText(1, this.pressure + "");
		itemPressure.setText(2, (new SiUnit("Pa")).toString());

		itemTemperatureF.setText(0, LocalizationHandler
				.getItem("app.dd.testing.gui.temperaturefluid"));
		itemTemperatureF.setText(1, this.temperatureF + "");
		itemTemperatureF.setText(2, (new SiUnit("K")).toString());

		itemMaterial.setText(0,
				LocalizationHandler.getItem("app.dd.testing.gui.coolant"));
	...
	private void addToTestingTable(Duct duct, String prefix, double flowRate,
			double temperatureIn, double pressureIn) {
		...
		for (ADuctElement e : duct.getElements()) {
			...
			itemProp.setText(4, String.format("%.3g",
					e.getPressureDrop(flowRate, pressureIn, temperatureIn)));
			itemProp.setText(5, String.format("%.3g", e
					.getPressureLossCoefficient(flowRate, pressureIn,
							temperatureIn)));
			itemProp.setText(
					6,
					String.format(
							"%.3g",
							e.getRth(flowRate, pressureIn, temperatureIn)
									* e.getSurface()));
			itemProp.setText(
					7,
					String.format("%.3g",
			...
			itemProp.setText(9, String.format("%.3g",
					e.getTemperatureOut(temperatureIn, flowRate, pressureIn)));
			itemProp.setText(10, String.format("%.3g",
					e.getWallTemperature(temperatureIn, flowRate, pressureIn)));

			if (e instanceof DuctBypass) {
				addToTestingTable(((DuctBypass) e).getPrimary(),
						prefix.replaceFirst("[0-9]:", "  ") + "  1: ",
						((DuctBypass) e).getFlowRatePrimary(flowRate,
								pressureIn, temperatureIn), temperatureIn,
						pressureIn);
				addToTestingTable(((DuctBypass) e).getSecondary(),
						prefix.replaceFirst("[0-9]:", "  ") + "  2: ",
						((DuctBypass) e).getFlowRateSecondary(flowRate,
								pressureIn, temperatureIn), temperatureIn,
						pressureIn);
			}
	...
	private void addToBCTable(Duct duct, String prefix, double flowRate,
			double temperatureIn, double pressureIn) {
		...
		for (ADuctElement e : duct.getElements()) {
			if (!(e instanceof DuctFitting)) {
				...
				if (e instanceof DuctBypass) {
					addToBCTable(((DuctBypass) e).getPrimary(),
							prefix.replaceFirst("[0-9]:", "  ") + "  1: ",
							((DuctBypass) e).getFlowRatePrimary(flowRate,
									pressureIn, temperatureIn), temperatureIn,
							pressureIn);
					addToBCTable(((DuctBypass) e).getSecondary(),
							prefix.replaceFirst("[0-9]:", "  ") + "  2: ",
							((DuctBypass) e).getFlowRateSecondary(flowRate,
									pressureIn, temperatureIn), temperatureIn,
							pressureIn);
	...
	private void updateTestingTable() {
		...
		addToTestingTable(duct, "", this.flowRate, this.temperatureF,
				this.pressure);
		...
		itemProp.setText(
				4,
				String.format("%.3g",
						duct.getPressureDrop(flowRate, pressure, temperatureF)));
		itemProp.setText(5, String.format("%.3g", duct
				.getPressureLossCoefficient(flowRate, pressure, temperatureF)));
		itemProp.setText(6, String.format("%.3g",
				duct.getThermalResistance(flowRate, pressure, temperatureF)));
		itemProp.setText(
				7,
				String.format("%.3g",
						duct.getHTC(flowRate, pressure, temperatureF)));
	...
	private void updateChartTesting() {
		...
		if (duct.getElements().size() == 0) {
			...
			ArrayList<Double> positionArr = getPositions(this.duct, 0), htcArr = getHTCs(
					this.duct, this.pressure, this.temperatureF, this.flowRate), pressureArr = getPressures(
					this.duct, this.pressure, this.temperatureF, this.flowRate), tempFluidArr = getTemperaturesOut(
					this.duct, this.pressure, this.temperatureF, this.flowRate), tempWallArr = getTemperaturesWall(
					this.duct, this.pressure, this.temperatureF, this.flowRate);
	...
	private void updateChartCC() {
		...
		for (int i = 0; i < N; i++) {
			...
			pressure[i] = duct.getPressureDrop(flowRate[i], this.pressure,
					this.temperatureF);
			htc[i] = duct.getThermalResistance(flowRate[i], this.pressure,
					this.temperatureF);
		}
	...
	private void addPump(final Pump pump) {
		...
		ILineSeries line = (ILineSeries) chartCC
				.getSeriesSet()
				.createSeries(
						SeriesType.LINE,
						LocalizationHandler
								.getItem("app.dd.testing.gui.analysis.characteristicdiag.pump")
								+ ": " + pump.getType());
		line.setYAxisId(1);
		...
		item.setText(LocalizationHandler
				.getItem("app.dd.testing.gui.analysis.characteristicdiag.removepump")
				+ ": '" + pump.getType() + "'");
	...
	private void removePump(String type) {
		...
		this.chartCC.getSeriesSet().deleteSeries(
				this.lineSeriesPumps.get(index).getId());
	...
	private void saveChartCC(Chart chart) {
		...
		fileChooser.setFilterExtensions(new String[] { "*.png", "*.jpg",
				"*.bmp" });
		...
		Image image = new Image(Display.getDefault(), chart.getBounds().width,
				chart.getBounds().height);
	...
	private void setPumpColors() {
		...
		for (ILineSeries l : this.lineSeriesPumps)
			l.setLineColor(new Color(Display.getCurrent(), new RGB(0,
					100 + 155 * i++ / this.lineSeriesPumps.size(), 0)));
	}
	...
	private ArrayList<Double> getPositions(Duct duct, double offset) {
		...
		for (ADuctElement e : duct.getElements()) {
			if (e instanceof DuctBypass) {
				positions.addAll(getPositions(((DuctBypass) e).getPrimary(),
						offset));
				if (positions.size() > 0)
	 ...
	 * @return
	 */
	private ArrayList<Double> getPressures(Duct duct, double pressureIn,
			double temperatureIn, double flowRate) {
		ArrayList<Double> pressures = new ArrayList<Double>();

		for (ADuctElement e : duct.getElements()) {
			if (e instanceof DuctBypass) {
				pressures.addAll(getPressures(((DuctBypass) e).getPrimary(),
						pressureIn, ((DuctBypass) e).getFlowRatePrimary(
								flowRate, pressureIn, this.temperatureF),
						temperatureIn));
				if (pressures.size() > 0)
					pressureIn = pressures.get(pressures.size() - 1);
			} else {
				pressureIn = e.getPressureOut(this.flowRate, pressureIn,
						temperatureIn);
	 ...
	 * @return
	 */
	private ArrayList<Double> getTemperaturesOut(Duct duct, double pressureIn,
			double temperatureIn, double flowRate) {
		ArrayList<Double> temperatures = new ArrayList<Double>();

		for (ADuctElement e : duct.getElements()) {
			if (e instanceof DuctBypass) {
				temperatures.addAll(getTemperaturesOut(((DuctBypass) e)
						.getPrimary(), pressureIn, temperatureIn,
						((DuctBypass) e).getFlowRatePrimary(flowRate,
								pressureIn, temperatureIn)));
			} else {
				temperatures.add(e.getTemperatureOut(temperatureIn, flowRate,
						pressureIn));
			}
	 ...
	 * @return
	 */
	private ArrayList<Double> getTemperaturesWall(Duct duct, double pressureIn,
			double temperatureIn, double flowRate) {
		ArrayList<Double> temperatures = new ArrayList<Double>();

		for (ADuctElement e : duct.getElements()) {
			if (e instanceof DuctBypass) {
				temperatures.addAll(getTemperaturesWall(((DuctBypass) e)
						.getPrimary(), pressureIn, temperatureIn,
						((DuctBypass) e).getFlowRatePrimary(flowRate,
								pressureIn, temperatureIn)));
			} else {
				temperatures.add(e.getWallTemperature(temperatureIn, flowRate,
						pressureIn));
			}
	 ...
	 * @return
	 */
	private ArrayList<Double> getHTCs(Duct duct, double pressureIn,
			double temperatureIn, double flowRate) {
		ArrayList<Double> htcs = new ArrayList<Double>();
		ArrayList<Double> pressures = getPressures(duct, pressureIn,
				temperatureIn, flowRate);
		ArrayList<Double> temperatures = getTemperaturesOut(duct, pressureIn,
				temperatureIn, flowRate);

		for (ADuctElement e : duct.getElements()) {
			if (e instanceof DuctBypass) {
				if (htcs.size() > 0)
					htcs.addAll(getHTCs(((DuctBypass) e).getPrimary(),
							pressures.get(htcs.size() - 1), temperatures
									.get(htcs.size() - 1), ((DuctBypass) e)
									.getFlowRatePrimary(flowRate, pressureIn,
											this.temperatureF)));
				else
					htcs.addAll(getHTCs(((DuctBypass) e).getPrimary(),
							pressureIn, temperatureIn, ((DuctBypass) e)
									.getFlowRatePrimary(flowRate, pressureIn,
											this.temperatureF)));
			} else {
				htcs.add(e.getRth(flowRate, pressures.get(htcs.size()),
						temperatures.get(htcs.size())) * e.getSurface());
			}
		}
		...
		for (ADuctElement e : duct.getElements())
			if (pos + e.getLength() > x) {
				if (e instanceof DuctBypass)
					return e.getName()
							+ ": "
							+ getElementName(((DuctBypass) e).getPrimary(), x
									- pos);
				else
					return e.getName();
			} else
