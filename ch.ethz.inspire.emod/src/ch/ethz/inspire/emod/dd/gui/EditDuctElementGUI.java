...
public class EditDuctElementGUI extends AConfigGUI {
	...
	private void updatePropertyTable() {
		...
		itemName.setText(0,
				LocalizationHandler.getItem("app.dd.elemet.gui.name"));
		...
		for (String key : element.getParameterSet().getParameterSet().keySet()) {
			...
			itemParam.setText(1, parametersNew.getPhysicalValue(key).getValue()
					+ "");
			itemParam.setText(2, parametersNew.getPhysicalValue(key).getUnit()
					.toString());
	...
	private void updateIsolationItem() {
		...
		itemIsolation.setText(0,
				LocalizationHandler.getItem("app.dd.elemet.gui.isolation"));
		...
		buttonEditIsolation.setLayoutData(new GridData(SWT.FILL, SWT.TOP,
				false, true, 1, 1));
		...
		buttonDeleteIsolation.setLayoutData(new GridData(SWT.FILL, SWT.TOP,
				false, true, 1, 1));
	...
	private void updateProfileItem() {
		...
		itemProfile.setText(0,
				LocalizationHandler.getItem("app.dd.elemet.gui.profile"));
		...
		buttonEditProfile.setLayoutData(new GridData(SWT.FILL, SWT.TOP, false,
				true, 1, 1));
	...
	private void editDuctProfile(ADuctElement element) {
		Shell shell = EditDuctProfileGUI.editDuctProfileGUI(this.getShell(),
				element);
	...
	private void editDuctIsolation(Isolation isolation) {
		Shell shell = EditDuctIsolationGUI.editDuctIsolationGUI(
				this.getShell(), isolation);
		shell.addDisposeListener(new DisposeListener() {
	...
	public void save() {
		...
		for (int i = 3; i < tableProperties.getItemCount(); i++) {
			try {
				parametersNew.setPhysicalValue(tableProperties.getItem(i)
	...
	public void reset() {
		for (String s : this.parametersNew.getParameterSet().keySet())
			this.parametersNew.setPhysicalValue(s, element.getParameterSet()
					.getPhysicalValue(s));
