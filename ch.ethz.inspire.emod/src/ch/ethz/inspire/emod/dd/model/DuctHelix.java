...
public class DuctHelix extends ADuctElement {
	 ...
	 * @param profile
	 */
	public DuctHelix(String name, double r, double h, double d,
	...
	private void init() {
		...
		N = height / distance;
		dl = Math.sqrt(Math.pow(distance, 2)
				+ Math.pow(2 * Math.PI * radius, 2));
	...
	public double getHTC(double flowRate, double pressure,
		...
		return Fluid.convectionForcedCoil(getMaterial(), temperatureWall,
				temperatureFluid, profile, radius * 2, distance, flowRate);
	...
	public double getPressureDrop(double flowRate, double pressure,
		...
		return Fluid.pressureLossFrictionCoil(getMaterial(), temperatureFluid,
				length, profile, 2 * radius, distance, flowRate);
	}
